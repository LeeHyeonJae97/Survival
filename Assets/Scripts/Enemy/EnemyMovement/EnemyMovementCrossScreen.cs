...
public class EnemyMovementCrossScreen : EnemyMovement
{
    ...
    public override void Movement_Start(EnemyPlayer enemy)
    {
        ...
        {
            enemy.transform.right = new Vector2(bounds.max.x, Random.Range(bounds.min.y, bounds.max.y)) - (Vector2)enemy.transform.position;
        }

        // from right to left
        else if (enemy.transform.position.x >= bounds.max.x)
        {
            enemy.transform.right = new Vector2(bounds.min.x, Random.Range(bounds.min.y, bounds.max.y)) - (Vector2)enemy.transform.position;
        }

        // from bottom to top
        else if (enemy.transform.position.y <= bounds.min.y)
        {
            enemy.transform.right = new Vector2(Random.Range(bounds.min.x, bounds.max.x), bounds.max.y) - (Vector2)enemy.transform.position;
        }

        // from top to bottom
        else if (enemy.transform.position.y >= bounds.max.y)
        {
            enemy.transform.right = new Vector2(Random.Range(bounds.min.x, bounds.max.x), bounds.min.y) - (Vector2)enemy.transform.position;
        }
    }
